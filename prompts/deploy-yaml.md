...
## `depends_on` は本当に必要な依存だけに使う

`depends_on` は「その順序が実際に必要な依存」（DB → アプリ、アプリ → LB など）にのみ設定する。`docker compose up --build` 再実行時のコンテナ再作成レース（`container ... is not connected to the network` 等）を避ける目的で、依存関係のないサービス同士を直列化しない（例: `app2` を `app1` に依存させる）。同じ理由で `build.provenance: false` / `build.sbom: false` も付けない。これらのレースはデプロイ実行側が再試行で対処する責務を持つ。
...
docker-compose.yml の内容のみを stdout に直接出力してください。ファイル書き込みツールは使用しないでください（init スクリプトを除く）。Markdown のコードフェンスで囲まないでください。ファイル内容の前後に説明文を含めないでください。

**1 行目は必ず `name:` または `services:` から始めること**（例: `name: myapp`）。「〜setup is done」「Here is the ...」のような状況説明を先頭・末尾・YAML 内に混ぜない。`name:` の値は空白を含まない 1 トークンにする。
